...
3. **Source of truth** — which of Linear/Jira/Confluence/Notion/Figma they use and
   ...
   guidance. For Codex, source MCPs are configured in `~/.codex/config.toml`
   `[mcp_servers.*]` (not `.mcp.json`) — offer to add the ones they want.
4. **Codex engine** — already running on Codex, so set `.tms/memory/codex.yaml`
   `codex_role: off` (the hybrid worker/reviewer roles are a Claude-side feature;
   on Codex the team runs natively).
5. **Style learning** (if cases exist) — sample 10-20 cases, draft `conventions.md`,
   confirm with the user.
6. **Glossary** — extract frequent domain terms, build `glossary.md`.
7. **Commit** — show the tree, get confirmation, write `.tms/memory/*` from the
   templates, and add `.tms/debug/` + `.tms/.codex-availability` to `.gitignore`.
