...
`kensa browser` verb set, the persistence model, and the report-back loop.
...
1. Run `kensa browser status --format json`.
...
1. Work through the routine body step by step, driving the browser with
   `kensa browser …` (always `--format json` for machine-readable output).
...
2. If the routine surfaced a **defect**, follow the `kensa-browser` report-back
   loop: file a case with `kensa new --suite bugs/<area> --title "…" --tag
