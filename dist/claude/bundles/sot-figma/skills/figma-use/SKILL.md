...
**In kensa-qa this skill is read-mostly**: QA uses `use_figma` to inspect deep node structure, prototype flows, and annotations programmatically — building or editing designs is out of scope for a QA session. Start from the read-oriented sections of the references below.
